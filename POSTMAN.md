# Guía de pruebas en Postman — Ecomarket SPA · E-commerce
...
La colección ya define estas variables (o créalas en un *Environment*):
...
## 2. Inicio-Sesion — Autenticación (JWT)
...
- **GET** `{{gateway}}/api/v1/productos`
- **200:** array con los 100 productos sembrados.

### 3.2 Obtener un producto por id
- **GET** `{{gateway}}/api/v1/productos/1`
- **200:**
  ```json
  {
...
- **GET** `{{gateway}}/api/v1/productos/buscar?nombre=solar`
- **200:** productos cuyo nombre contiene "solar" (sin importar mayúsculas).

### 3.4 Filtrar por categoría
- **GET** `{{gateway}}/api/v1/productos/categoria/MASCOTAS`
...
### 3.5 Crear un producto
- **POST** `{{gateway}}/api/v1/productos`
- **Body (raw JSON):**
  ```json
  {
    "sku": "ALI-99",
...
- **PUT** `{{gateway}}/api/v1/productos/1`
...
- **DELETE** `{{gateway}}/api/v1/productos/101`
- **204 No Content** si se elimina · **404** si no existe.

---

## 4. Catálogo — Inventario (stock)

### 4.1 Verificar disponibilidad
- **GET** `{{gateway}}/api/v1/inventario/1/disponibilidad?cantidad=5`
- **200:** `{ "disponible": true }`

### 4.2 Reservar stock (descuenta)
- **POST** `{{gateway}}/api/v1/inventario/1/reservar?cantidad=5`
- **200:** `{ "stockRestante": 95 }`
- **409:** si no hay stock suficiente · **404:** si el producto no existe.

### 4.3 Liberar stock (devuelve)
- **POST** `{{gateway}}/api/v1/inventario/1/liberar?cantidad=5`
- **200:** `{ "stockRestante": 100 }`

### 4.4 Ajustar stock (gerente)
- **PUT** `{{gateway}}/api/v1/inventario/1/stock?nuevaCantidad=200`
...
### 5.1 Ver / crear el carrito de un cliente
- **GET** `{{gateway}}/api/v1/carritos/1`
- **200:** el carrito activo del cliente 1 (se crea vacío si no existía).

### 5.2 Agregar un producto al carrito
- **POST** `{{gateway}}/api/v1/carritos/items`
- **Body:**
  ```json
...
### 5.3 Eliminar un ítem del carrito
- **DELETE** `{{gateway}}/api/v1/carritos/1/items/1`  (el segundo número es el `id` del ítem)
- **200:** carrito actualizado (se libera el stock reservado) · **404** si el ítem no existe.

### 5.4 Vaciar el carrito
- **DELETE** `{{gateway}}/api/v1/carritos/1`
...
- ⚠️ **Regla de negocio:** los productos comprados **se eliminan del registro del
  Catálogo-Inventario** (si luego los consultas por id → 404). El pedido conserva el
  nombre y precio como *snapshot*.

### 6.2 Cancelar un pedido
- **POST** `{{gateway}}/api/v1/compras/1/cancelar`  (el número es el `id` del pedido)
- **200:** pedido con `estado: "CANCELADO"` (restaura el stock si el producto aún existe) · **404** si no existe.

### 6.3 Historial de pedidos
- **GET** `{{gateway}}/api/v1/compras/historial/1`
...
**Prueba de resiliencia:** apaga el catálogo (Ctrl+C en su terminal) y repite el paso 3.
...
Además de Postman, cada microservicio expone su documentación interactiva
(**springdoc-openapi**). Con el servicio arriba, abre en el navegador:
